package diglol.crypto

import diglol.crypto.random.nextBytes
import kotlin.random.Random
import kotlinx.benchmark.Benchmark
import kotlinx.benchmark.BenchmarkMode
import kotlinx.benchmark.BenchmarkTimeUnit
import kotlinx.benchmark.Measurement
import kotlinx.benchmark.Mode
import kotlinx.benchmark.OutputTimeUnit
import kotlinx.benchmark.Param
import kotlinx.benchmark.Scope
import kotlinx.benchmark.Setup
import kotlinx.benchmark.State
import kotlinx.coroutines.test.runTest

@State(Scope.Benchmark)
@Measurement(iterations = 5, time = 1)
@BenchmarkMode(Mode.AverageTime)
@OutputTimeUnit(BenchmarkTimeUnit.MICROSECONDS)
class Ed25519Benchmark {
  @Param("1024", "10240", "102400")
  var factor = 0
  private lateinit var data: ByteArray

  private lateinit var keyPair: KeyPair
  private lateinit var signature: ByteArray

  @Setup
  fun setup() = runTest {
    data = Random.nextBytes(factor)
    keyPair = Ed25519.generateKeyPair(nextBytes(32))
    signature = Ed25519.sign(keyPair.privateKey, data)
  }

  @Benchmark
  fun generateKeyPairWithPrivateKey() = runTest {
    Ed25519.generateKeyPair(keyPair.privateKey)
  }

  @Benchmark
  fun sign() = runTest {
    Ed25519.sign(keyPair.privateKey, data)
  }

  @Benchmark
  fun verify() = runTest {
    Ed25519.verify(signature, keyPair.publicKey, data)
  }
}
